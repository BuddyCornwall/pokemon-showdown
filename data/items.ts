...
name: "Cat's Toy",
onStart(pokemon) {
const boostedMons = ['gossifleur','toxel'];
...
if (!['psyduck','farfetchd','farfetchd-galar','jynx','magby','magmar','magmortar','porygon','porygon2','porygon-z','mew','blaziken','lotad','lombre','ludicolo','pelipper','gothita','gothorita','gothitelle','ducklett','cramorant','quaxly','quaxwell','quaquaval','wattrel'].includes(target.species.name)) {
...
name: "Marmalade",
onStart(pokemon) {
const boostedMons = ['pyroar'];
if (boostedMons.includes(pokemon.species.id)) {
this.heal(Math.floor(pokemon.maxhp / 20), pokemon);
}
},
},
...
name: "Roman's Reigns",
onStart(pokemon) {
const boostedMons = ['rockruff','dubwool','boltund'];
if (boostedMons.includes(pokemon.species.id)) {
this.boost({spe: 1}, pokemon);
}
},
