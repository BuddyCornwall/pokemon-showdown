...
perishsong: {
    ...
    onHit(target, source, move) {
        let duration = this.random(2, 5); // Random duration between 2 and 5 turns
        this.add('-start', target, 'perish' + duration);
        target.addVolatile('perishsong', target, move, 'perishsong', duration);
        this.add('-fieldactivate', 'move: Perish Song');
    },
    condition: {
        duration: 5, // Maximum duration, it will be adjusted individually for each Pokémon
        onStart(target) {
        ...
        onResidual(pokemon) {
            ...
            if (duration > 0) {
                ...
                this.add('-start', pokemon, 'perish0');
                pokemon.faint();
            }
        },
    },
